...
from netbalance.configs.vibrio import (
    VIBRIO_BACTERIA_NAMES_FILE,
    VIBRIO_PHAGE_NAMES_FILE,
    VIBRIO_DATASET_FILE,
)
...
class VibrioDataset(ADataset):
    ...
    def get_cluster_a_node_names(self):
        names = pd.read_csv(VIBRIO_BACTERIA_NAMES_FILE, index_col=0)
        return list(names.iloc[:, 0])

    def get_cluster_b_node_names(self):
        names = pd.read_csv(VIBRIO_PHAGE_NAMES_FILE, index_col=0)
        return list(names.iloc[:, 0])

    def get_dataset_file_path(self):
        return VIBRIO_DATASET_FILE
