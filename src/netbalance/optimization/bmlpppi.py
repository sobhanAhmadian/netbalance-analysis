...
from netbalance.configs.bmlpppi import BMLPPPIOptimizerConfig
...
from netbalance.models.bmlpppi import BMLPPPIModelHandler
...
class BMLPPPITrainer(Trainer):

    def train(
        self,
        model_handler: BMLPPPIModelHandler,
        data: BGData,
        config: BMLPPPIOptimizerConfig,
    ) -> Result:
...
class BalanceBMLPPPITrainer(Trainer):
    ...
    def train(
        ...
        def task(num_bal):
            save_name = (
                f"i{config.i_balance_method}_bmlpppi_{hash_associations}_{num_bal}"
            )
            logger.info(
