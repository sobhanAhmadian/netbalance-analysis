...
from netbalance.configs.bmlpphi import BMLPPHIOptimizerConfig
...
from netbalance.models.bmlpphi import BMLPPHIModelHandler
...
class BMLPPHITrainer(Trainer):

    def train(
        self,
        model_handler: BMLPPHIModelHandler,
        data: BGData,
        config: BMLPPHIOptimizerConfig,
    ) -> Result:
        ...
        if config.save:
            if config.save_path is not None:
                model_handler.save_model(config.save_path)

        return result


class BalanceBMLPPHITrainer(Trainer):
    ...
    def train(
        ...
        def task(num_bal):
            save_name = (
                f"i{config.i_balance_method}_bmlpphi_{hash_associations}_{num_bal}"
            )
        ...
        tasks = [
            dask.delayed(task)(num_bal) for num_bal in range(0, config.i_max_num_bal)
        ]
        ...
        if config.i_parallel_balance:
            with (
                ...
                Client(
                    LocalCluster(
                        n_workers=min(
                            int(os.getenv("NUM_WORKERS")), config.i_max_num_bal
                        ),
