...
for i in range(num_strain_features + num_phage_features):
    beta_top_mask = beta_mean_abs_shap >= np.sort(beta_mean_abs_shap)[-i - 1]
    irho_top_mask = irho_mean_abs_shap >= np.sort(irho_mean_abs_shap)[-i - 1]
...
fig.tight_layout()
file_name = f"{figs_folder}/bar-top-n.svg"
