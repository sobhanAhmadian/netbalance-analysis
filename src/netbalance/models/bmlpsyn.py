...
from netbalance.configs.bmlpsyn import (
    BMLPSYN_CELL_FEATURES_DIR,
    BMLPSYN_DRUG_FEATURES_DIR,
    BMLPSYNModelConfig,
)
...
class BMLPSYNFeatureExtractor(FeatureExtractor):
    ...
    def __init__(
        ...
        self.drug_file = Path(f"{BMLPSYN_DRUG_FEATURES_DIR}/{drug_feature_name}.txt")
        self.cell_file = Path(f"{BMLPSYN_CELL_FEATURES_DIR}/{cell_feature_name}.txt")
...
class BMLPSYNModelHandler(AModelHandler):
    ...
    def _build_feature_extractor(self):
        return BMLPSYNFeatureExtractor(
            self.model_config.drug_feature_name, self.model_config.cell_feature_name
        )


class BMLPSYNHandlerFactory(HandlerFactory):
    def __init__(self, model_config: BMLPSYNModelConfig) -> None:
        super().__init__()
        self.model_config = model_config

    def create_handler(self) -> BMLPSYNModelHandler:
        return BMLPSYNModelHandler(self.model_config)
